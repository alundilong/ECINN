...
This GPU-oriented version generates training batches directly on the selected
PyTorch device and preloads experimental flux interpolation tables as torch
tensors. This avoids CPU NumPy/SciPy interpolation inside every optimization
step.

Recommended CUDA command:

```bash
python -m ecinn_pytorch.train_ecinn --device cuda --batch-size 2048 --steps-per-epoch 500 --epochs 300 --torch-num-threads 0
```

For larger GPUs, try `--batch-size 4096` or `--batch-size 8192`. PINNs with
second derivatives are usually memory-limited before they are compute-limited.
